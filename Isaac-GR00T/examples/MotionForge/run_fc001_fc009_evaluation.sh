...
GROOT_MODEL_PATH="${GROOT_MODEL_PATH:-${WORKSPACE_ROOT}/ckpts/MotionforgeGroup/gr00t1.7/FC-80000}"
...
GROOT_ACCEL_MODE="${GROOT_ACCEL_MODE:-pytorch}"
...
FC_EVAL_CLOCK_MODE="${FC_EVAL_CLOCK_MODE:-wall_clock_strict}"
FC_EVAL_ACTION_ALIGNMENT="${FC_EVAL_ACTION_ALIGNMENT:-observation_aligned}"
OBS_PORT="${FC_EVAL_OBS_PORT:-3196}"
ACT_PORT="${FC_EVAL_ACT_PORT:-3198}"
CLIENT_WARMUP_S="${FC_EVAL_CLIENT_WARMUP_S:-5}"
...
RUN_ID="${FC_EVAL_RUN_ID:-fc001_fc009_$(date +%Y%m%d_%H%M%S)}"
...
log() {
  printf '[FC001-FC009-EVAL] %s\n' "$*"
}

die() {
  printf '[FC001-FC009-EVAL] ERROR: %s\n' "$*" >&2
  exit 1
}
...
validate_configuration() {
  ...
  require_uint_at_least "FC_EVAL_CLIENT_WARMUP_S" "${CLIENT_WARMUP_S}" 0
  ...
  [[ "${FC_EVAL_CLOCK_MODE}" == "wall_clock_strict" ]] || die \
    "FC_EVAL_CLOCK_MODE must be wall_clock_strict for realtime FC evaluation; got ${FC_EVAL_CLOCK_MODE}"
  [[ "${FC_EVAL_ACTION_ALIGNMENT}" == "observation_aligned" ]] || die \
    "FC_EVAL_ACTION_ALIGNMENT must be observation_aligned for realtime FC evaluation; got ${FC_EVAL_ACTION_ALIGNMENT}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  log "tasks=${#TASK_IDS[@]} trials_per_task=${NUM_TRIALS} max_steps=${max_steps_label} motion_level=${MOTION_LEVEL:-benchmark_config} initial_position_mode=${INITIAL_POSITION_MODE} clock_mode=${FC_EVAL_CLOCK_MODE} action_alignment=${FC_EVAL_ACTION_ALIGNMENT} physics_device=${MOTIONFORGE_DEVICE} video_outcome_suffix=${VIDEO_OUTCOME_SUFFIX} expected_trials=$((${#TASK_IDS[@]} * NUM_TRIALS))"
...
mkdir -p "${RESULT_DIR}"
{
  printf 'FC001-FC009 MotionForge evaluation\n'
  printf 'started_at=%s\n' "$(date --iso-8601=seconds)"
  printf 'model=%s\n' "${GROOT_MODEL_PATH}"
  printf 'accel_mode=%s\n' "${GROOT_ACCEL_MODE}"
