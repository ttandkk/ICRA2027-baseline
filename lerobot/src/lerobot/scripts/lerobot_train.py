...
def update_policy(
    ...
    trace_first_step = os.environ.get("FIRST_STEP_TRACE", "0").lower() in {"1", "true", "yes", "on"}

    def trace(phase: str) -> None:
        if not trace_first_step:
            return
        rank = accelerator.process_index
        print(f"FIRST_STEP_TRACE rank={rank} {phase} start", flush=True)
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        print(f"FIRST_STEP_TRACE rank={rank} {phase} done elapsed_s={time.perf_counter() - start_time:.3f}", flush=True)
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    # FastWAM and the baseline policies train all parameters in their standard
    # objective. Avoiding unused-parameter scans saves an autograd traversal
    # per step, while bucket views prevent DDP from holding a second full
    # gradient-sized communication buffer for large policies.
    if accelerator is None:
        # Accelerate creates its FSDP plugin from the launcher environment.
        # DDP-specific kwargs must not be installed in FSDP mode.
        is_fsdp_launch = os.environ.get("ACCELERATE_USE_FSDP", "false").lower() in {
            "1", "true", "yes", "on"
        }
        ...
        if is_fsdp_launch:
            fsdp_plugin = FullyShardedDataParallelPlugin(
                ...
                transformer_cls_names_to_wrap=["MoTLayer"],
        ...
        # Keep FP32 FSDP parameter shards for FastWAM numerical stability while
        # Accelerate runs forward/backward and gradient communication in BF16.
        accelerator = Accelerator(
    ...
    for _ in range(step, cfg.steps):
        start_time = time.perf_counter()
        trace_first_step = (
            step == 0 and os.environ.get("FIRST_STEP_TRACE", "0").lower() in {"1", "true", "yes", "on"}
        )
        if trace_first_step:
            print(f"FIRST_STEP_TRACE rank={accelerator.process_index} batch_fetch_enter", flush=True)
        batch = next(dl_iter)
        if trace_first_step:
            print(f"FIRST_STEP_TRACE rank={accelerator.process_index} batch_fetch_done", flush=True)
        ...
        if trace_first_step:
            print(f"FIRST_STEP_TRACE rank={accelerator.process_index} preprocess_done elapsed_s={time.perf_counter() - start_time:.3f}", flush=True)
