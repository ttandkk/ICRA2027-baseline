...
class TrainPipelineConfig(HubMixin):
    ...
    def validate(self) -> None:
        ...
        # Under torchrun/accelerate every rank validates independently. Only
        # rank 0 may reject an existing fresh-run directory; it can create
        # W&B files while other ranks are still validating.
        is_main_process = int(os.environ.get("RANK", "0")) == 0
        if not self.resume and is_main_process and isinstance(self.output_dir, Path) and self.output_dir.is_dir():
