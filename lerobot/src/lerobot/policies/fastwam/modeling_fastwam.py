...
def _initialize_module_on_device(module_type, config: dict[str, Any], device: str, dtype: torch.dtype):
    """Construct a large module directly on its target device and dtype.

    FastWAM's video DiT has about five billion parameters. Constructing it with
    PyTorch's CPU/float32 defaults creates a large transient host-memory copy
    before ``.to(cuda, bfloat16)`` can release it. This is prohibitively large
    on GPU partitions with a modest per-job RAM limit.
    """
...
class FastWAMPolicy(PreTrainedPolicy):
    ...
    def _batch_to_training_sample(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        if "image_is_pad" not in sample:
            # LeRobot keeps one temporal padding mask per camera. FastWAM trains
            # on their horizontally concatenated video, so a timestep is invalid
            # when any constituent camera had to be episode-end padded.
            camera_pad_masks = [
                batch[f"{key}_is_pad"]
            ...
            if camera_pad_masks:
                sample["image_is_pad"] = torch.stack(camera_pad_masks, dim=0).any(dim=0)
    ...
    def _build_core_model(self, config: FastWAMConfig) -> FastWAM:
        ...
        video_expert = _initialize_module_on_device(WanVideoDiT, config.video_dit_config, device, dtype)
        action_expert = _initialize_module_on_device(ActionDiT, config.action_dit_config, device, dtype)
        mot = MoT(
...
def _stack_video_from_images(batch: dict[str, Tensor], config: FastWAMConfig) -> Tensor:
    ...
    # Preserve the configured camera order. It defines the left-to-right layout
    # of the concatenated image and must be identical in training and inference.
    image_keys = [key for key in config.image_features if key in batch]
    if not image_keys:
