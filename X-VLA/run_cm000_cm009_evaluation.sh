#!/usr/bin/env bash
#SBATCH --job-name=xvla_cm000_cm009_eval
#SBATCH --partition=cluster02
#SBATCH --gres=gpu:rtx5090:1
#SBATCH --time=48:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=48G
#SBATCH --output=logs/xvla_cm000_cm009_eval_%j.out
#SBATCH --error=logs/xvla_cm000_cm009_eval_%j.err
...
OBS_PORT="${XVLA_EVAL_OBS_PORT:-43396}"
ACT_PORT="${XVLA_EVAL_ACT_PORT:-43398}"
...
RESULT_ROOT="${XVLA_EVAL_OUTPUT_ROOT:-${SCRIPT_DIR}/output/circular_motion/cm000_cm009/level2_fixed/server_scheduled}"
RUN_ID="${XVLA_EVAL_RUN_ID:-xvla_cm000_cm009_$(date +%Y%m%d_%H%M%S)}"
RESULT_DIR="${RESULT_ROOT}/${RUN_ID}"
SUMMARY_FILE="${RESULT_DIR}/success_rates.txt"
DRY_RUN="${DRY_RUN:-0}"
OOD_LIGHTING="${XVLA_EVAL_OOD_LIGHTING-0}"
OOD_SPEED="${XVLA_EVAL_OOD_SPEED-0}"
...
log() {
  printf '[XVLA-CM000-CM009-EVAL] %s\n' "$*"
}

die() {
  printf '[XVLA-CM000-CM009-EVAL] ERROR: %s\n' "$*" >&2
  exit 1
}
...
validate_configuration() {
  ...
  [[ "${OOD_LIGHTING}" == "0" || "${OOD_LIGHTING}" == "1" ]] \
    || die "XVLA_EVAL_OOD_LIGHTING must be 0 or 1"
  ...
  [[ "${OOD_SPEED}" == "0" || "${OOD_SPEED}" == "1" ]] \
    || die "XVLA_EVAL_OOD_SPEED must be 0 or 1"
  if [[ "${OOD_SPEED}" == "1" ]]; then
    ((10#${START_SEED} < 50 && 10#${NUM_TRIALS} <= 50 - 10#${START_SEED})) \
      || die "speed OOD requires all trial seeds to be in 0-49"
    [[ -z "${MOTION_LEVEL}" ]] \
      || die "XVLA_EVAL_OOD_SPEED cannot be combined with XVLA_EVAL_MOTION_LEVEL"
  fi
...
mkdir -p "${RESULT_DIR}"
{
  printf 'X-VLA CM000-CM009 MotionForge evaluation\n'
  printf 'started_at=%s\n' "$(date --iso-8601=seconds)"
  printf 'model=%s\n' "${XVLA_MODEL_PATH}"
  printf 'device=%s\n' "${XVLA_DEVICE}"
