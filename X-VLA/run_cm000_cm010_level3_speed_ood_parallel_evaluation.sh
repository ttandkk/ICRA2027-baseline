...
NUM_TRIALS="${XVLA_CM_LEVEL3_EVAL_NUM_TRIALS:-50}"
START_SEED="${XVLA_CM_LEVEL3_EVAL_START_SEED:-0}"
GPU_LIST="${XVLA_CM_LEVEL3_EVAL_GPUS:-3}"
...
LANE_TWO_ACT_PORT="${XVLA_CM_LEVEL3_EVAL_LANE2_ACT_PORT:-3498}"
...
log() {
  printf '[XVLA-CM-LEVEL3-SPEED-OOD] %s\n' "$*"
}

die() {
  printf '[XVLA-CM-LEVEL3-SPEED-OOD] ERROR: %s\n' "$*" >&2
  exit 1
}
...
validate_configuration() {
  ...
  [[ "${BATCH_ID}" =~ ^[A-Za-z0-9._-]+$ ]] \
    || die "XVLA_CM_LEVEL3_EVAL_BATCH_ID may contain only letters, numbers, dot, underscore, and hyphen"
  require_uint_at_least "XVLA_CM_LEVEL3_EVAL_NUM_TRIALS" "${NUM_TRIALS}" 1
  require_uint_at_least "XVLA_CM_LEVEL3_EVAL_START_SEED" "${START_SEED}" 0
  ...
  if [[ "${DRY_RUN}" == "0" ]]; then
    ...
    validate_runtime_resources
    mkdir -p "${OUTPUT_ROOT}" || die "failed to create X-VLA output directory: ${OUTPUT_ROOT}"
  fi
}
...
create_level3_motionforge_root() {
  ...
  TEMP_MOTIONFORGE_ROOT="$(mktemp -d /tmp/xvla_cm_level3.XXXXXXXXXX)" \
...
run_lane() {
  ...
  exec env \
    ...
    XVLA_EVAL_OUTPUT_ROOT="${OUTPUT_ROOT}" \
    XVLA_EVAL_TASKS="${tasks}" \
    XVLA_EVAL_RUN_ID="${run_id}" \
