...
SMOLVLA_MODEL_PATH="${SMOLVLA_MODEL_PATH:-${WORKSPACE_ROOT}/ckpts/MotionforgeGroup/SmolVLA/FC-80000/pretrained_model}"
...
MAX_STEPS="${SMOLVLA_EVAL_MAX_STEPS:-1400}"
USE_BENCHMARK_MAX_STEPS="${SMOLVLA_EVAL_USE_BENCHMARK_MAX_STEPS:-1}"
MOTION_LEVEL="${SMOLVLA_EVAL_MOTION_LEVEL:-}"
INITIAL_POSITION_MODE="${SMOLVLA_EVAL_INITIAL_POSITION_MODE:-fixed}"
OBS_PORT="${SMOLVLA_EVAL_OBS_PORT:-3196}"
ACT_PORT="${SMOLVLA_EVAL_ACT_PORT:-3198}"
...
RUN_ID="${SMOLVLA_EVAL_RUN_ID:-smolvla_fc001_fc009_$(date +%Y%m%d_%H%M%S)}"
...
log() {
  printf '[SMOLVLA-FC001-FC009-EVAL] %s\n' "$*"
}

die() {
  printf '[SMOLVLA-FC001-FC009-EVAL] ERROR: %s\n' "$*" >&2
  exit 1
}
...
' "${SMOLVLA_MODEL_PATH}" || die "checkpoint config does not match the trained SmolVLA-FC contract"
}
...
validate_configuration() {
  ...
  require_uint_at_least "SMOLVLA_EVAL_MAX_STEPS" "${MAX_STEPS}" 1
  ...
  [[ "${USE_BENCHMARK_MAX_STEPS}" == "0" || "${USE_BENCHMARK_MAX_STEPS}" == "1" ]] \
    || die "SMOLVLA_EVAL_USE_BENCHMARK_MAX_STEPS must be 0 or 1"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  log "tasks=${#TASK_IDS[@]} trials_per_task=${NUM_TRIALS} max_steps=${max_steps_label} motion_level=${MOTION_LEVEL:-benchmark_config} initial_position_mode=${INITIAL_POSITION_MODE} send_horizon=${SMOLVLA_SEND_HORIZON} execution_horizon=${SMOLVLA_EXECUTION_HORIZON}"
...
mkdir -p "${RESULT_DIR}"
{
  printf 'SmolVLA FC001-FC009 MotionForge evaluation\n'
  printf 'started_at=%s\n' "$(date --iso-8601=seconds)"
  printf 'model=%s\n' "${SMOLVLA_MODEL_PATH}"
  printf 'device=%s\n' "${SMOLVLA_DEVICE}"
