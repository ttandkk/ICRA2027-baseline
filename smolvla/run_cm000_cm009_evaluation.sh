...
OOD_SPEED="${SMOLVLA_EVAL_OOD_SPEED-0}"
...
validate_configuration() {
  ...
  [[ "${OOD_SPEED}" == "0" || "${OOD_SPEED}" == "1" ]] \
    || die "SMOLVLA_EVAL_OOD_SPEED must be 0 or 1"
  if [[ "${OOD_SPEED}" == "1" ]]; then
    ((10#${START_SEED} < 50 && 10#${NUM_TRIALS} <= 50 - 10#${START_SEED})) \
      || die "speed OOD requires all trial seeds to be in 0-49"
    [[ -z "${MOTION_LEVEL}" ]] \
      || die "SMOLVLA_EVAL_OOD_SPEED cannot be combined with SMOLVLA_EVAL_MOTION_LEVEL"
  fi
