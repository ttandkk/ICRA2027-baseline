#!/usr/bin/env bash
#SBATCH --job-name=smolvla_hri_eval
#SBATCH --partition=cluster02
#SBATCH --gres=gpu:rtx5090:1
#SBATCH --time=48:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=48G
#SBATCH --output=logs/smolvla_hri_eval_%j.out
#SBATCH --error=logs/smolvla_hri_eval_%j.err
...
SMOLVLA_MODEL_PATH="${SMOLVLA_MODEL_PATH:-${WORKSPACE_ROOT}/ckpts/SmolVLA-HRI-80000}"
...
SMOLVLA_HF_HOME="${SMOLVLA_HF_HOME:-${HF_HOME:-${HOME}/.cache/huggingface}}"
...
SMOLVLA_EVAL_CUDA_VISIBLE_DEVICES="${SMOLVLA_EVAL_CUDA_VISIBLE_DEVICES:-${CUDA_VISIBLE_DEVICES:-0}}"
TIMEOUT_EXE="${MOTIONFORGE_TIMEOUT_EXE:-$(command -v timeout || true)}"

START_SEED="${SMOLVLA_EVAL_START_SEED:-0}"
NUM_TRIALS="${SMOLVLA_EVAL_NUM_TRIALS:-5}"
ATTEMPTS_PER_WORKER="${SMOLVLA_EVAL_ATTEMPTS_PER_WORKER:-5}"
HRI006_ATTEMPTS_PER_WORKER="${SMOLVLA_EVAL_HRI006_ATTEMPTS_PER_WORKER:-1}"
...
OBS_PORT="${SMOLVLA_EVAL_OBS_PORT:-3496}"
ACT_PORT="${SMOLVLA_EVAL_ACT_PORT:-3498}"
...
RESULT_ROOT="${SMOLVLA_EVAL_OUTPUT_ROOT:-${SCRIPT_DIR}/output/human_robot_interaction/hri000_hri009/level2_fixed/server_scheduled}"
RUN_ID="${SMOLVLA_EVAL_RUN_ID:-smolvla_hri_scoring_smoke_$(date +%Y%m%d_%H%M%S)}"
...
log() {
  printf '[SMOLVLA-HRI000-HRI009-EVAL] %s\n' "$*"
}

die() {
  printf '[SMOLVLA-HRI000-HRI009-EVAL] ERROR: %s\n' "$*" >&2
  exit 1
}
...
validate_checkpoint() {
  ...
  PYTHONOPTIMIZE= PYTHONDONTWRITEBYTECODE=1 "${SMOLVLA_PYTHON}" -c '
import json
import sys
...
assert config.get("type") == "smolvla", config.get("type")
...
assert preprocessor.get("steps", [None] * 6)[5].get("registry_name") == "normalizer_processor"
assert postprocessor.get("steps", [None])[0].get("registry_name") == "unnormalizer_processor"
' "${SMOLVLA_MODEL_PATH}" || die "checkpoint does not match the trained SmolVLA-HRI contract"
}
...
' || die "SmolVLA runtime imports failed; verify LeRobot and pyzmq"
}
...
assert list(inspect.signature(client.SmolVLAInference.predict).parameters) == ["self", "observation"]
assert BenchmarkClientBridge is not None
' "${SCRIPT_DIR}" || die "SmolVLA bridge does not match the server-scheduled contract"
}
...
validate_configuration() {
  ...
  require_uint_at_least "SMOLVLA_EVAL_HRI006_ATTEMPTS_PER_WORKER" "${HRI006_ATTEMPTS_PER_WORKER}" 1
  [[ "${HRI006_ATTEMPTS_PER_WORKER}" == "1" ]] || die \
    "SMOLVLA_EVAL_HRI006_ATTEMPTS_PER_WORKER must be 1 because particle cloth is not resettable"
  ...
  [[ "${VIDEO_OUTCOME_SUFFIX}" == "1" ]] \
    || die "SMOLVLA_EVAL_VIDEO_OUTCOME_SUFFIX must be 1 for result validation"
  [[ "${SMOLVLA_EVAL_CUDA_VISIBLE_DEVICES}" =~ ^[0-9]+$ ]] \
    || die "SMOLVLA_EVAL_CUDA_VISIBLE_DEVICES must select one CUDA device index"
  [[ "${RUN_ID}" =~ ^[A-Za-z0-9._-]+$ ]] \
...
print_bridge_command() {
  printf '  (cd %q && CUDA_VISIBLE_DEVICES=%q HF_HOME=%q PYTHONOPTIMIZE= PYTHONDONTWRITEBYTECODE=1 HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PYTHONPATH=%q ' \
    "${LEROBOT_ROOT}" "${SMOLVLA_EVAL_CUDA_VISIBLE_DEVICES}" "${SMOLVLA_HF_HOME}" "${BRIDGE_PYTHONPATH}"
...
mkdir -p "${RESULT_DIR}"
{
  printf 'SmolVLA HRI000-HRI009 MotionForge scoring smoke\n'
  printf 'started_at=%s\n' "$(date --iso-8601=seconds)"
  printf 'model=%s\n' "${SMOLVLA_MODEL_PATH}"
  printf 'smolvla_device=%s\n' "${SMOLVLA_DEVICE}"
