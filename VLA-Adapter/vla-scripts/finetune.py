...
from prismatic.vla.datasets import LeRobotBatchTransform, LeRobotV3Dataset, RLDSDataset, RLDSBatchTransform
...
class FinetuneConfig:
    ...
    dataset_format: str = "rlds"                      # `rlds` or `lerobot_v3`
    lerobot_image_key: str = "observation.images.front"
    lerobot_wrist_image_key: str = "observation.images.wrist"
    lerobot_task_instruction: Optional[str] = None    # Shared instruction when metadata has no task text
    lerobot_task_instructions_path: Optional[Path] = None  # Optional JSON: {task_index: instruction}
    ...
    enable_gradient_checkpointing: bool = True        # Trade compute for lower activation memory
    ...
    # Logging
    wandb_entity: Optional[str] = None               # Optional W&B entity; uses the logged-in account when unset
    wandb_project: str = "vla-adapter"              # Name of W&B project
    wandb_mode: str = "online"                      # W&B mode: online, offline, or disabled
...
def run_forward_pass(
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        output: CausalLMOutputWithPast = vla(
            ...
            use_film=use_film,
            skip_lm_head=use_l1_regression,
            )
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.enable_gradient_checkpointing:
        ...
        # KV caching is incompatible with activation checkpointing during training.
    ...
    if cfg.dataset_format == "rlds":
    ...
    elif cfg.dataset_format == "lerobot_v3":
        ...
        train_dataset = LeRobotV3Dataset(
            cfg.data_root_dir / cfg.dataset_name,
            batch_transform,
            image_key=cfg.lerobot_image_key,
            wrist_image_key=cfg.lerobot_wrist_image_key,
            task_instruction=cfg.lerobot_task_instruction,
            task_instructions_path=cfg.lerobot_task_instructions_path,
        )
    else:
        raise ValueError(f"Unsupported dataset_format={cfg.dataset_format!r}; expected `rlds` or `lerobot_v3`")

    if cfg.use_val_set and cfg.dataset_format == "rlds":
    ...
    elif cfg.use_val_set:
        raise ValueError("LeRobot v3 datasets currently expose only the train split; set --use_val_set False")
    ...
    dataloader = DataLoader(
        ...
        # RLDS shuffles internally; map-style LeRobot datasets need PyTorch shuffling.
        shuffle=(cfg.dataset_format == "lerobot_v3"),
        sampler=None,
        collate_fn=collator,
    ...
    # RLDS repeats internally. LeRobot v3 is finite, so repeat complete (and
    # freshly shuffled) epochs until the requested optimizer-step budget is met.
    def repeat_train_batches():
        while True:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        vla.train()
        optimizer.zero_grad()
        for batch_idx, batch in enumerate(repeat_train_batches()):
            ...
            if not torch.isfinite(normalized_loss):
                print(f"Skipping batch {batch_idx}: non-finite loss ({loss.detach().float().item()}).")
                optimizer.zero_grad()
                continue
            ...
            # batch_idx is zero-based; this is the number of completed optimizer
            # updates for grad_accumulation_steps=1.
            ...
            log_step = (cfg.resume_step or 0) + gradient_step_idx if cfg.resume else gradient_step_idx
            ...
            if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    trainable_params, max_norm=cfg.max_grad_norm, error_if_nonfinite=False
                )
                if not torch.isfinite(grad_norm):
                    print(f"Skipping optimizer step {gradient_step_idx}: non-finite gradient norm ({grad_norm.item()}).")
                    optimizer.zero_grad()
                    continue
                optimizer.step()
                scheduler.step()
    ...
    # Save the terminal state even when max_steps is not divisible by save_freq.
    final_step = (cfg.resume_step or 0 if cfg.resume else 0) + (batch_idx + 1) // cfg.grad_accumulation_steps
    if final_step > 0 and final_step != last_checkpoint_step:
        save_training_checkpoint(
            cfg=cfg,
            run_dir=run_dir,
            log_step=final_step,
