...
def _time_ms(fn, *, device: torch.device | str) -> tuple[Any, float]:
    # Wall-clock timing (includes CPU launch overhead). We still synchronize around the region
    # for CUDA so the measured time corresponds to actual work completion.
...
class PI0Pytorch(nn.Module):
    def __init__(self, config):
        ...
        enable_compile = os.environ.get("OPENPI_ENABLE_TORCH_COMPILE", "0").lower() in {"1", "true", "yes"}
        if enable_compile:
            self.sample_actions = torch.compile(self.sample_actions, mode="max-autotune")

        # Stage-level compilation toggles used by sample_actions_with_timing.
        # These are intentionally separate from OPENPI_ENABLE_TORCH_COMPILE so you can compile
        # only specific stages for A/B timing tests.
        self._encoder_stage = self._encoder_stage_impl
        self._vlm_prefill_stage = self._vlm_prefill_stage_impl
        self._action_stage = self._action_stage_impl
    ...
    def sample_actions_with_timing(self, device, observation, noise=None, num_steps=10) -> tuple[Tensor, dict[str, float]]:
        ...
        # 1) encoder stage (embed_prefix: images + language tokens)
        ...
        # 2) VLM prefix KV prefill
        past_key_values, ms = _time_ms(
            lambda: self._vlm_prefill_stage(prefix_embs, prefix_pad_masks, prefix_att_masks),
            device=device,
        )
        timing["vlm_prefill_ms"] = ms

        # 3) action denoise loop
        actions, ms = _time_ms(
            lambda: self._action_stage(state, prefix_pad_masks, past_key_values, noise, num_steps),
            device=device,
        )
        timing["action_ms"] = ms

        timing["total_ms"] = float(timing["encoder_ms"] + timing["vlm_prefill_ms"] + timing["action_ms"])
        return actions, timing

    # ---- Stage implementations (used by sample_actions_with_timing; may be torch.compile'd) ----
