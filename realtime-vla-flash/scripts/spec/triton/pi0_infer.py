    ...
    while pid < grid_m * grid_n:
        ...
        for k in range(0, features, BLOCK_SIZE_K):
            ...
            x = tl.load(
                ...
                mask = (offs_i < seq_len) & (offs_k[None, :] < features), other = 0
            )
            # x = x * norm_factor[:, None]
            w = tl.load(
        ...
        if start_j < qk_cols:
            ...
            x0_ = x0 * x_cos - x1 * x_sin
            x1_ = x1 * x_cos + x0 * x_sin
            rotated = tl.interleave(x0_, x1_)
            accumulator = tl.where(offs_j < qk_cols, rotated, accumulator)
...
class Pi0Inference:
    def __init__(self, checkpoint, num_views, chunk_size):
        ...
        self.weights = {
            ...
            "decoder_action_time_mlp_in_w":        torch.empty(2048, 1024,             dtype = torch.bfloat16, device = "cuda"),
            "decoder_action_time_mlp_in_b":        torch.empty(1024,                   dtype = torch.bfloat16, device = "cuda"),
        ...
        if not all(key in checkpoint for key in exact_verify_keys):
            for key in exact_verify_keys:
